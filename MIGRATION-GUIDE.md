...
このドキュメントでは、藍の各バージョン間での移行手順を説明します。

## memory.json から SQLite への移行

藍は以前 LokiJS (memory.json) を使用していましたが、現在は SQLite データベースを使用しています。

### 自動移行

藍を起動すると、`memory.json` が存在する場合は自動的に SQLite へ移行されます。
...
移行が完了すると：
- 新しい `ai.db` ファイルが作成されます
- `ai.db.migrated` マーカーファイルが作成されます
- 元の `memory.json` は保持されます（バックアップとして）

移行を確認した後、`memory.json` を削除しても構いません。
